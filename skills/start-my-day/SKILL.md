...
- 将当前 `SKILL.md` 的父目录作为 `$SKILL_DIR`。
- Windows PowerShell 使用 `$env:OBSIDIAN_VAULT_PATH`，macOS/Linux shell 使用 `$OBSIDIAN_VAULT_PATH`。
- 运行脚本时优先使用当前可用的 `python`（Windows）或 `python3`（macOS/Linux）；不要假定固定解释器路径。
- 首次运行前验证 `python -c "import yaml, requests, fitz"` 成功；缺少依赖时安装项目 `requirements.txt` 中的依赖。
- 在 Windows 上若环境变量尚未进入当前进程，从配置文件的 `vault_path` 读取 Vault 路径，不要回退到 Linux 示例路径。
...
Windows PowerShell 等价配置读取：

```powershell
$configPath = Join-Path $env:OBSIDIAN_VAULT_PATH '99_System\Config\research_interests.yaml'
$LANGUAGE = (Select-String -Path $configPath -Pattern '^\s*language:' | Select-Object -First 1).Line.Split(':', 2)[1].Trim().Trim('"')
if (-not $LANGUAGE) { $LANGUAGE = 'zh' }
$NOTE_SUFFIX = if ($LANGUAGE -eq 'en') { 'paper-recommendations' } else { '论文推荐' }
```
...
2. **读取研究配置**
   ...
   - 提取：关键词、类别和优先级
   - 以 Zotero 馆藏中的标题、标签和期刊分布作为兴趣画像；当前画像重点是地震勘探数据处理、混采与去混采、去噪与重建、时频分析、速度分析、波场模拟，以及物理约束/深度学习地球物理
   - 优先关注 `GEOPHYSICS`、`IEEE Transactions on Geoscience and Remote Sensing`、`IEEE Geoscience and Remote Sensing Letters`、`Geophysical Journal International`、`Geophysical Prospecting`、`Interpretation` 和 `Journal of Applied Geophysics`
...
python scripts/search_arxiv.py \
  ...
  --max-results 200 \
  --top-n 10 \
  --download-dir "$OBSIDIAN_VAULT_PATH/20_Research/Papers/"
```
...
- **Links**: [arXiv](url) | [[local filename|PDF]]
- **Source**: arXiv
...
- 笔记文件中的 PDF 格式必须是 `[[本地文件名|PDF]]`，本地文件名必须链接到 `$OBSIDIAN_VAULT_PATH/20_Research/Papers/` 目录下的本地 PDF 文件

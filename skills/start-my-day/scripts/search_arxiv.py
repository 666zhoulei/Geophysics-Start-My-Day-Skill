...
SEMANTIC_SCHOLAR_FIELDS = "title,abstract,publicationDate,citationCount,influentialCitationCount,url,authors,authors.affiliations,externalIds,venue,openAccessPdf"

# 默认分类关键词映射（当配置中无用户自定义关键词时使用）
ARXIV_CATEGORY_KEYWORDS = {
    "physics.geo-ph": "geophysics seismology seismic exploration",
    "physics.comp-ph": "computational geophysics wavefield simulation",
    "eess.SP": "seismic signal processing",
    "eess.IV": "seismic imaging inversion",
    "cs.LG": "machine learning for geophysics",
    "math.NA": "numerical methods for wave equations",
}

DEFAULT_GEOPHYSICS_CATEGORIES = (
    "physics.geo-ph,physics.comp-ph,eess.SP,eess.IV,cs.LG,math.NA"
)
    ...
    try:
        ...
        return {
            "research_domains": {
                "地震勘探数据处理": {
                    "keywords": [
                        "seismic exploration", "seismic data processing", "deblending",
                        "simultaneous source", "seismic denoising", "seismic reconstruction"
                    ],
                    "arxiv_categories": DEFAULT_GEOPHYSICS_CATEGORIES.split(','),
                    "priority": 10
                }
            },
            "preferred_venues": ["GEOPHYSICS", "IEEE Transactions on Geoscience and Remote Sensing"],
            "excluded_keywords": ["workshop"]
        }


def categories_from_config(config: Dict) -> List[str]:
    """Return a stable, de-duplicated category list from research domains."""
    categories = []
    for domain in config.get('research_domains', {}).values():
        for category in domain.get('arxiv_categories', []):
            if category and category not in categories:
                categories.append(category)
    return categories or DEFAULT_GEOPHYSICS_CATEGORIES.split(',')


def safe_pdf_filename(paper: Dict) -> str:
    """Build a stable Obsidian-friendly local PDF filename."""
    arxiv_id = (paper.get('arxiv_id') or paper.get('arxivId') or '').replace('/', '_')
    stem = arxiv_id or title_to_note_filename(paper.get('title', 'paper'))[:120]
    return f"{stem}.pdf"
...
def calculate_relevance_score(
    ...
    for domain_name, domain_config in domains.items():
        ...
        matched_categories = [cat for cat in domain_categories if cat in categories]
        strong_category_match = 'physics.geo-ph' in matched_categories

        # Broad categories such as cs.LG/eess.IV span many unrelated fields.
        # They may reinforce a keyword match, but must not select a paper alone.
        if dm_keywords or strong_category_match:
            for cat in matched_categories:
                score += RELEVANCE_CATEGORY_MATCH_BOOST
                dm_keywords.append(cat)

        priority = max(1, min(10, int(domain_config.get('priority', 5))))
        score *= 0.8 + (priority / 25.0)
...
def filter_and_score_papers(
    ...
    for paper in papers:
        ...
        venue = (paper.get('venue') or paper.get('journal_ref') or '').lower()
        preferred_venues = [str(v).lower() for v in config.get('preferred_venues', [])]
        if venue and any(preferred in venue for preferred in preferred_venues):
            relevance = min(SCORE_MAX, relevance + 0.5)
            matched_keywords = matched_keywords + [paper.get('venue') or paper.get('journal_ref')]
...
def main():
    ...
    parser.add_argument('--categories', type=str,
                        default='',
                        help='Comma-separated arXiv categories; defaults to categories in the config')
    parser.add_argument('--download-dir', type=str, default='',
                        help='Directory that must receive a local PDF for every selected paper')
    ...
    logger.info("Using arXiv categories: %s", categories)
